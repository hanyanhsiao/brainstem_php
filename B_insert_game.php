...
if (isset($_FILES['gameCover'])) {
    ...
    // 執行插入語句
    $sql = "INSERT INTO GAME_DATA (GAME_NAME, GAME_COVER, GAME_INTRO, ORIGINAL_PRICE, GAME_STATUS, RELEASE_DATE, PUBLISHER, DEVELOPER, RATING_ID, SYSTEM_REQUIREMENT, TOTAL_COMMENTS, PURCHASED) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $statement = $pdo->prepare($sql);
    $statement->execute([$gameName, $gameCover, $gameIntro, $originalPrice, $gameStatus, $releaseDate, $publisher, $developer, $ratingID, $systemRequirement, $totalComments, $purchased]);
